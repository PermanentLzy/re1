...
static int foldBinary(int lhs, int rhs, const std::string& op) {
    ...
    if (op == "*")  return lhs * rhs;
    if (op == "%") {
        if (rhs == 0) throw std::runtime_error("常量折叠: 模零错误");
        return lhs % rhs;
    }
    if (op == "/") {
        if (rhs == 0) throw std::runtime_error("常量折叠: 除零错误");
        return lhs / rhs;
    }
    ...
    while (changed) {
        ...
        for (auto& instr : program.instructions) {
            ...
            if (instr.type == TACType::ASSIGN &&
                ...
                if (!name.empty()) constMap[name] = instr.lhs.intValue;
            }
            // ASSIGN result = src (复写) → 传播
                ...
                if (it != constMap.end()) {
                    ...
                    if (!dstName.empty()) constMap[dstName] = it->second;
                }
            }
            // 非常量写入 → 失效
            if (instr.type == TACType::ASSIGN &&
                instr.lhs.type != TACOpType::CONST_INT) {
                std::string name = opName(instr.result);
                if (!name.empty()) invalidated.insert(name);
            }
        ...
        for (auto& name : invalidated) constMap.erase(name);
            ...
            if (instr.type == TACType::BINARY) {
                ...
                if (lc && rc) {
                    try {
                        ...
                        changed = true; ++totalFolded; continue;
                    } catch (...) {}
                }
                // 同变量: x==x→1, x!=x→0, x<x→0, x<=x→1, x-x→0
                if (same) {
                    if (instr.op=="==" || instr.op=="<=" || instr.op==">=") {
                        instr.type=TACType::ASSIGN; instr.lhs=TACOperand::constInt(1);
                        instr.rhs=TACOperand::none(); instr.op.clear();
                        changed=true; ++totalFolded; continue;
                    }
                    if (instr.op=="!=" || instr.op=="<" || instr.op==">") {
                        instr.type=TACType::ASSIGN; instr.lhs=TACOperand::constInt(0);
                        instr.rhs=TACOperand::none(); instr.op.clear();
                        changed=true; ++totalFolded; continue;
                    }
                    if (instr.op=="-") {
                ...
                if (rc && rv == 0 && (instr.op == "+" || instr.op == "-")) {
                    instr.type = TACType::ASSIGN; instr.rhs = TACOperand::none(); instr.op.clear();
                    changed = true; ++totalFolded; continue;
                }
                // 0+x→x
                if (lc && lv == 0 && instr.op == "+") {
                    instr.type = TACType::ASSIGN;
                    instr.lhs = instr.rhs; instr.rhs = TACOperand::none(); instr.op.clear();
                    changed = true; ++totalFolded; continue;
                }
                // x*1→x
                if (rc && rv == 1 && instr.op == "*") {

    ...
    while (!check(TokenType::EOF_TOKEN)) {
        if (check(TokenType::CONST)) {
            ...
            if (check(TokenType::LPAREN)) {
                ...
                // 变量声明 (ToyC 必须初始化)
                consume(TokenType::ASSIGN, MSG("声明必须初始化: '='", "Declaration must be initialized: '='."));
...
std::unique_ptr<Stmt> Parser::parseVarDecl() {
    // VarDecl -> "int" ID "=" Expr ";"
    // ⚠️ ToyC 要求必须初始化, "=" 不是可选的
    DataType type = parseType();
    Token name = consume(TokenType::IDENTIFIER, MSG("需要变量名", "Expected variable name."));
    consume(TokenType::ASSIGN, MSG("变量声明必须初始化: '='", "Variable must be initialized: '='."));
    auto init = parseExpression();
...
std::unique_ptr<BlockStmt> Parser::parseBlock() {       // 代码块由一系列语句组成，直到遇到右大括号 '}'
    // TODO: consume(LBRACE);
...
std::unique_ptr<Expr> Parser::parseExpression() {
    return parseLogicalOr();     // 最低优先级入口：逻辑或
}
